...
def Path_gradient(numpy_image, model, idx_scale, attr_objective, path_interpolation_func, cuda=False):
    """
    ...
    for i in range(image_interpolation.shape[0]):
        ...
        img_tensor.requires_grad_(True)
        if cuda:
            result = model(_add_batch_one(img_tensor).cuda(), idx_scale)
            target = attr_objective(result)
            target.backward()
            ...
            result = model(_add_batch_one(img_tensor), idx_scale)
            target = attr_objective(result)
            target.backward()
            grad = img_tensor.grad.numpy()
            if np.any(np.isnan(grad)):
