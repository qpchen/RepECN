...
class one_conv(nn.Module):
    def __init__(self,inchanels,growth_rate,kernel_size = 3, relu = True):
        ...
        self.weight1 = esrt_common.Scale(1)
        self.weight2 = esrt_common.Scale(1)
    def forward(self,x):
        if self.flag == False:
...
class one_module(nn.Module):
    def __init__(self, n_feats):
        ...
        self.weight1 = esrt_common.Scale(1)
        self.weight2 = esrt_common.Scale(1)
        self.weight3 = esrt_common.Scale(1)
        self.weight4 = esrt_common.Scale(1)
        self.weight5 = esrt_common.Scale(1)
    def forward(self, x):
...
class Un(nn.Module):
    def __init__(self,n_feats, wn):
        ...
        self.reduce = esrt_common.default_conv(3*n_feats, n_feats, 3)
        self.weight2 = esrt_common.Scale(1)
        self.weight1 = esrt_common.Scale(1)
        self.attention = MLABlock(n_feat=n_feats, dim=288) 
        self.alise = esrt_common.default_conv(n_feats, n_feats, 3)
...
class ESRT(nn.Module):
    def __init__(self, upscale=4, conv=esrt_common.default_conv):
        ...
        rgb_std = (1.0, 1.0, 1.0)
        # self.sub_mean = esrt_common.MeanShift(args.rgb_range, rgb_mean, rgb_std)
        ...
        modules_tail = [
            ...
            esrt_common.Upsampler(conv, scale, n_feats, act=False),
            conv(n_feats, 3, kernel_size)]


        self.up = nn.Sequential(esrt_common.Upsampler(conv,scale,n_feats,act=False),
    ...
    def load_state_dict(self, state_dict, strict=False):
        ...
        #MSRB_out = []from model import esrt_common
