...
class GhostNetV2(nn.Module):
    ...
    def __init__(self, args, num_classes=1000, width=1.0, dropout=0.2, block=GhostBottleneckV2):
        ...
        self.conv_stem = nn.Conv2d(args.n_colors, output_channel, 3, 1, 1, bias=False)
        ...
        self.deconv = nn.Sequential(
            common.Upsampler(conv, args.scale[0], output_channel, act=False), #'prelu'),
            conv(output_channel, args.n_colors, 3)
        )
        upscale_factor = args.scale[0]
        num_channels = args.n_colors
        # self.deconv = nn.ConvTranspose2d(output_channel, num_channels, (9, 9), (upscale_factor, upscale_factor),
...
#          ]
#     ]
#     self.body = GhostNetV2(cfgs, num_classes=kwargs['num_classes'],
#                       width=kwargs['width'],
