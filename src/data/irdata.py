...
class IRData(data.Dataset):
    def __init__(self, args, name='', train=True, benchmark=False):
        self.args = args
        self.name = name.replace('_IR','')
        self.train = train
        ...
        self.degradation = args.degradation
        ...
        if args.ext.find('img') >= 0 or benchmark:
            ...
            if self.degradation == 'bicubic' or self.degradation == 'BD' or self.degradation == 'DN':
                for s in self.scale:
                    os.makedirs(
                        os.path.join(
            ...
            elif self.degradation == 'Noise' or self.degradation == 'Gray_Noise':
                os.makedirs(
                    os.path.join(
                        self.dir_lr.replace(self.apath, path_bin),
                        'N{}'.format(self.sigma)
            ...
            elif self.degradation == 'JPEG':
                os.makedirs(
                    os.path.join(
                        self.dir_lr.replace(self.apath, path_bin),
                        'Q{}'.format(self.quality)
                    ),
                    exist_ok=True
                )
            elif self.degradation == 'Blur':
                os.makedirs(
                    os.path.join(self.dir_lr.replace(self.apath, path_bin),'K25N2'),
                    exist_ok=True
                )
        ...
        if self.degradation == 'bicubic' or self.degradation == 'BD' or self.degradation == 'DN':
            ...
            for f in names_hr:
                ...
                names_lr[0].append(os.path.join(
                    self.dir_lr, 'N{}/{}n{}{}'.format(
                        self.sigma, filename, self.sigma, self.ext[1]
                    )
                ))
        elif self.degradation == 'JPEG':
            names_lr = [[]]
            for f in names_hr:
                filename, _ = os.path.splitext(os.path.basename(f))
                names_lr[0].append(os.path.join(
                    self.dir_lr, 'Q{}/{}q{}{}'.format(
                        self.quality, filename, self.quality, self.ext[1]
                    )
                ))
        elif self.degradation == 'Blur':
            names_lr = [[]]
            for f in names_hr:
                filename, _ = os.path.splitext(os.path.basename(f))
                names_lr[0].append(os.path.join(
                    self.dir_lr, 'K25N2/{}k25n2{}'.format(
                        filename, self.ext[1]
                    )
                ))
    ...
    def _set_filesystem(self, dir_data):
        ...
        if self.degradation == "Gray_Noise":
            self.dir_hr = os.path.join(self.apath, 'HR_Gray')
        else:
            self.dir_hr = os.path.join(self.apath, 'HR')
        self.dir_lr = os.path.join(self.apath, 'LR_{}'.format(self.degradation))
    ...
    def __getitem__(self, idx):
        ...
        if lr.ndim == 2 and hr.ndim == 2:
            #hr = np.expand_dims(hr, axis=2) #np.repeat(lr[..., np.newaxis], 1, axis=-1)
